...
| --- | --- | --- |
| 线管 | `ρ = 1.35` (固定) | |
| 水带 | `ρ = 1.35` (固定, 与线管相同) | 2026-08-01 老板确认 |
| 钢丝管 | `ρ = inner_diameter × 0.003 + 1.46` | 钢丝型号会影响, 粗略按口径估 |
| 复合管 | `ρ = inner_diameter × 0.003 + 1.46` (与钢丝管相同) | 2026-08-01 老板确认 |
| 塑筋管 | (TODO 待客户补充) | |

> **原始类别映射（2026-08-01 新增）**：`products.product_category` 存客户原始类别（70+ 种，如"无味钢丝管""白复合防静电""钩编管"），密度计算前先经 `tools/csv_to_sql.py::CATEGORY_ALIASES` 映射成上表 4 个大类；映射表没覆盖的按名称关键词兜底（含"钢丝"→钢丝管、含"复合"→复合管、含"水带"→水带），仍判不了返回 None 跳过校验。
...
### 5.1 反向校验（2026-08-01 起降级为 WARN，保留客户值）

下列字段，客户手填值跟公式算的理论值偏差 > 5% → **报 WARN 提醒，保留客户手填值不改动**（`mismatch_level: "warn"`，偏差提示由导入脚本写入 `remark`）：
...
**背景（2026-08-01 真实主数据导入约定，见 `BUSINESS_RULES.md R4`）**：客户给的厚度/米重/单重是业务承诺值，允许"上下浮动"超 5%（如实收称重、工艺差异），系统无权擅自改数。所以超差只提醒、不阻止生成 SQL。**注意**：几何派生字段（外径 `outer_diameter`、体积 `volume`）不在此列，手填超差仍是 ERROR 阻止生成（见 `derived-fields` skill §6）。

**历史变更**：2026-08-01 之前这三个字段超差是 ERROR 阻止生成；真实主数据导入（1.4 万行客户手填值）后降级为 WARN。
...
- `DENSITY_RULES` 字典里加新条目（新**大类**时）
- 客户来了新的**原始类别**名 → 在 `CATEGORY_ALIASES` 加映射（归到 4 个大类之一），不用动 `DENSITY_RULES`
- 本 skill 文档第 1 节加新行
...
- ✅ 客户给的米重/单重/厚度偏差 > 5%，**WARN 提醒、保留客户值**（2026-08-01 起），导入脚本把偏差写入 remark，不要擅自改数
...
反向校验 (填了的字段都跟公式比 5%):
   ...
   ├─ 偏差 > 5% (厚度/米重/单重): WARN 提醒, 保留客户值 (2026-08-01 起)
   └─ 偏差 > 5% (外径/体积等几何派生): ERROR, 阻止生成 SQL (见 derived-fields)
   │
   ▼
跨字段互校:
   │

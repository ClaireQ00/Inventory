...
-- ------------------------------------------------------------
DROP TABLE IF EXISTS aux_purchase_requests;
...
-- ------------------------------------------------------------
-- 10.6 辅料采购需求 aux_purchase_requests (2026-08-02 老板:
-- 合同录入缺料提示可"下推采购需求单"; 只登记需求不联动库存,
-- 到货后走 aux_stock_moves(source_type='purchase') 入库消化,
-- 状态人工流转 pending→ordered→received)
-- ------------------------------------------------------------
DROP TABLE IF EXISTS aux_purchase_requests;
CREATE TABLE aux_purchase_requests (
    id              INT AUTO_INCREMENT PRIMARY KEY COMMENT '内部ID',
    ...
    source_no       VARCHAR(32)  NOT NULL DEFAULT '' COMMENT '来源单号(如合同号)',

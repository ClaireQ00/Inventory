...
> **测试数据铁律（2026-08-14 T7001 事件后定）**：e2e/演示测试只能用专用编码段——
> 客户 `Z9999`、物料 `M-Z9999-*`、单据号 `*-TEST*`；测试脚本开跑前必须做冲突预检，
> 撞上任何现存数据立即中止。绝不用真实客户/物料编码段做测试。
> 现成脚本：`python3 tests/demo_roleplay_test.py`（41 项断言 + 自动清理）。

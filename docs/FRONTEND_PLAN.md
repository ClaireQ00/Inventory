...
> - **物料全量重编码**（2026-08-11 老板规则）：`M-NNNNN` 纯流水/`M-Q025` 旧式 → `M-{客户编码}-{3位流水}` 共 14,345 行（工具 `tools/renumber_materials.py`，单事务关 FK 检查改 products+10 张引用表，MySQL+CSV 双轨）；顺带归一小写/全角客户码 34 行、`8039.0`→D8039、`9129.0` 9 行挂 X9129（老板定）；7 行孤儿保留旧码（老板明确不动）；映射留痕 `data/logs/material_remap_20260811.csv`；`suggest-material-id` 已接续新格式。
> - **Streamlit 边界裁决**（2026-08-11 老板问"查询时顺手录值不值得破例"）：**不做双份录入表单**——维护双份界面必然漂移、同事"该用哪个"的认知成本、当初砍 Streamlit 录入的原因（全页重跑/明细编辑不可行）仍在；改为基础资料页加 **"➕ 新增"跳转按钮**（产品→/entry/product、客户→/entry/customer、业务员→/entry/salesperson），目标地址用 `ENTRY_BASE` 环境变量（局域网改宿主机 IP），拿到"少一次切换"90% 的体验、架构零污染。供应商/仓库录入页待补（FB.x）。

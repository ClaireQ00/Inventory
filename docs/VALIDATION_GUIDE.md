...
| 5/16 | 发货数 ≤ 合同数（校验时顺便把现算实发数**回写 `delivered_qty`**，字段从此可信） | 不能发超过合同约定的货 |
...
| 15/16 | 报价金额一致 + 派生关系完整性（主表=Σ明细小计、**主表 total_volume=Σ 明细 total_volume**、formal 必须从 brief 派生、converted 回填合同、subtotal=重量×系数×数量、**快照重量分阶段提醒：brief 偏离主数据 WARN 提示新增物料；formal/converted 偏离 WARN 要求归位到正确物料编码(WARN)**） | 报价单是合同的"前身",数字必须自洽 |
| 16/16 | **Packing Plan 公斤价反算**（报价系数×单重 = 应等于的合同单价(原币/件)，差 ≤ 0.01 正常，超差 warn；取数快照优先：converted 来源报价 → 同客户最新报价(非draft优先) → products 主数据兜底；覆盖有发货明细的合同行(回写 doi) + **未发货合同行(WARN 带 [合同未发货] 前缀)**） | 标准成本差异分析：报价是基准,合同是实际,允许微小四舍五入差 |

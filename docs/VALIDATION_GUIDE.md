...
脚本会自动生成一套**假的演示数据**（物料号都是 `DEMO-`，客户名都是"客户A"），跑完 7 步校验，告诉你整套流程是怎么连起来的。
...
校验逻辑写在 `tools/local_validator.py` 里，对应进销存真实业务流程：

| 步骤 | 校验内容 | 类比 |
| --- | --- | --- |
| 1/7 | 基础资料完整性（物料/仓库/供应商/客户） | 把通讯录建好 |
| 2/7 | 采购单总金额 = 明细小计之和 | 购物车总价 = 商品单价 × 数量加起来 |
| 3/7 | 入库数 ≤ 采购数 | 不能收超过下单的货 |
| 4/7 | 销售合同总金额 = 明细小计之和 | 同上，只是方向相反 |
| 5/7 | 发货数 ≤ 合同数 | 不能发超过合同约定的货 |
| 6/7 | 累计出库 ≤ 累计入库（同一仓库同一物料） | 银行取款不能超过存款 |
| 7/7 | 库存表 = 流水累加 | 账户余额 = 流水累加 |

...
你的 Excel/Numbers ──(另存CSV)──> data/csv/*.csv ──(一键脚本)──> 业务校验报告
                                       ↑                              ↓
                              只放在本地，被 .gitignore 忽略     14 步端到端检查
```
...
脚本会自动生成一套**假的演示数据**（物料号都是 `DEMO-`，客户名都是"客户A"），跑完 14 步校验，告诉你整套流程是怎么连起来的。
...
| 步骤 | 校验内容 | 类比 |
| --- | --- | --- |
| 1/14 | 基础资料完整性（物料/仓库/供应商/客户） | 把通讯录建好 |
| 2/14 | 采购单总金额 = 明细小计之和 | 购物车总价 = 商品单价 × 数量加起来 |
| 3/14 | 入库数 ≤ 采购数 | 不能收超过下单的货 |
| 4/14 | 销售合同总金额 = 明细小计之和 | 同上，只是方向相反 |
| 5/14 | 发货数 ≤ 合同数 | 不能发超过合同约定的货 |
| 6/14 | 累计出库 vs 累计入库（同一仓库同一物料，**负库存报警**） | 银行卡透支,允许但要提醒补钱 |
| 7/14 | 库存表 = 流水累加（对账） | 账户余额 = 流水累加 |
| 8/14 | 明细表体积小计（外径/体积派生） | 货柜装多少货要算清楚 |
| 9/14 | 报关实际数 vs 发货单计划数（UCP600 ±5% 容差） | 海关账跟合同账允许 5% 误差 |
| 10/14 | 贷记单闭环（pending 不能挂超过 30 天） | 短装赔偿必须收尾 |
| 11/14 | 汇率表完整性（每月每币种至少一条） | 每月 1 号录汇率,整月用这条 |
| 12/14 | 收款 vs 合同金额（按原币种聚合） | 客户付的款要跟合同对得上 |
| 13/14 | 调拨配对（同 `transfer_ref` 出入库数量必须相等） | 转账流水号两边必须平 |
| 14/14 | 报价金额一致 + 派生关系完整性（主表=Σ明细小计、formal 必须从 brief 派生、converted 回填合同、subtotal=重量×系数×数量） | 报价单是合同的"前身",数字必须自洽 |

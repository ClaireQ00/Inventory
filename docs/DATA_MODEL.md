...
| 26 | `aux_materials` | 生产辅料 | 辅料+工艺档案库(标签纸/包装/喷码/米标/用料/打线/盘型/其他; 标签纸计库存, 其余纯档案不计量; name 255 兼容喷码长文) | — |
...
- **关键字段**:
  - 标识:`material_id`(企业内部唯一,**编码规则 `M-{客户编码}-{3位流水}`,如 M-Q0025-001**;2026-08-11 全量重编码,14,345 行从旧 M-NNNNN 纯流水/M-Q025 旧式归入新格式(含 `9129.0` 9 行挂 X9129,老板定),映射留痕 `data/logs/material_remap_20260811.csv`;仅 7 行孤儿保留旧码,老板明确不动)、`customer_code`、`product_category`(决定密度公式)
...
#### `salespersons` — 业务员档案(2026-08-11 新增)
- **职责**:客户编码规则的权威来源 + 后期客户管理/仓位/回款/提成/统计的锚点。
- **编码规则**(老板定):客户编码 = **字母(业务员 `code`) + 4 位数字**(客户终身号;首位数字 = 该业务员数字编码 `digit`,即"首次把客户引入系统的业务员")。客户换业务员**只换字母、数字不变**(所以 A8039→D8039 是同一客户沿革)。录入校验 `^[A-Z]\d{4}$`。
- **关键字段**:`code`(首字母,UNIQUE)、`name`、`digit`(首位数字)、`commission_rate`(提成比例,预留)、`is_active`
- **编码推荐**:`suggest_customer_code(letter)` = 字母 + digit + 3 位流水(序列内 max+1;新业务员空序列从 001 起,"找相应的空值推荐");字母无档案时不给建议,先建业务员。
- **种子**:16 个字母按存量 596 个客户编号统计回填(A8/B6/C0/D1/E9/F5/H7/K7/L4/P0/Q0/T7/W~~9~~**3**/X9/Y2/Z9,**W 周世增 2026-08-11 老板定 9→3 避开重复**,存量 W9xxx 客户编码不动、新 W 客户从 W3001 起)。**姓名已补 10 人(2026-08-11 老板提供)**:A 周思宝、B 黄传宝、D 刘栋、F 丰海功、H 陈焕、L 王玉良、Q 郄桂鑫、W 周世增、X 赵晓卫、Y 岳勇;C/E/K/P/T/Z 待补。
- **管理界面**:`/entry/salesperson` 业务员档案页(2026-08-11)——列表(含停用)/建档/编辑;`code` 与 `digit` 是客户编码的锚,**建档后不可改**(API 层拒改),换业务员走客户编码换字母。编辑接口 `PUT /api/salespersons/{code}`,全字段列表 `GET /api/salespersons`。

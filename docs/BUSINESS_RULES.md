...
| 2026-08-14 | 合同行级关闭 | 新增:客户可只放弃合同某一行(不必整合同关单);sales_contract_items.status active/closed,closed 行不计入还欠/未发完/缺口统计且禁止再发货;所有行了结合同自动 completed;关闭走 API+审计,8501 合同页有按钮 |
| 2026-08-14 | 低价先发货拦截 | 由 WARN 升级为 ERROR:同客户同物料存在单价更高的旧合同未发完时,禁止给低价新合同开发货单;确需先发须老板特批(发货头 price_gap_approved=true + price_gap_reason 必填,原因随审计留痕);8082 发货录入页有"老板特批"勾选项;8501 首页预警卡同步提示 |
| 2026-08-14 | 关行约束(轻量版) | 关闭合同行原因必填(此前可空);放弃余量>5%的关行照常放行但进 8501 首页"大额余量关行复核"卡,老板事后复核。**预留(待登录权限上线)**:余量>5%的关行改为事前审批——业务员提交→老板审核通过才生效;无权限系统时审批是摆设,故只做"留痕+公示" |

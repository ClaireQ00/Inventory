...
- **包装方式档案（M1c, 2026-08-02）**：products.package 全部 102 种历史包装种子进 aux_materials（PK-001..102 按引用频次冻结，纯档案不计量、不进收发存）；录入页"包装/标签纸"改辅料档案下拉（可手填新值）。
- **手填值自动建档·总开关（M1d, 2026-08-02）**：aux_type 扩展 spray_code/meter_mark/material_used/wire_pattern/coil_type 五类工艺档案（纯档案不计量，Q7 定案不变）+ name 加宽 255（喷码长文不截断）。录入页**一个总复选框**（默认开）管 7 个字段：包装/标签纸/喷码/米标/用料/打线/盘型，手填新值在物料入库成功后自动建档（独立小事务，失败只告警），重复值静默跳过。各字段下拉=档案库+客户历史合并。

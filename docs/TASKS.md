...
- [x] M1c [P1] **包装方式入辅料档案 + 录入页下拉**（2026-08-02 老板要求）：products.package 全部 102 种历史包装种子进 aux_materials（PK-001..102 按引用频次冻结快照，纯档案不计量、不进收发存，种子文件幂等复跑 ✓）；**录入页"包装"和"标签纸"改辅料档案下拉**（AutoComplete 可手填新值，标签纸显示"R码·名称"存原 R 编号）；auxMaterials 接口加 aux_type 过滤参数。**手填新包装自动建档开关**（M1c+）：录入页复选框默认开，insert_row 加 options 透传，物料入库成功后独立小事务建档（失败不拖垮物料入库，只告警）；实测：开关开→新包装自动建档 PK-103 ✓、开关关→不建档 ✓、同名包装不重复建档 ✓（测试数据已清理，基线 products=14350/mats=105 恢复）

...
- [x] ~~FC.1 [P1] **C 期**：报价/合同/发货录入~~ **已被 F2.6 覆盖完成**（2026-08-02 录入端主线收官，明细行编辑+快照重量规则均已落地），此条为重复登记，勾销
- [ ] F3.1 [P2] **打印导出**：报价单/合同/发货单/出库单/对账单/报关资料的模板导出——**等老板提供模板**（格式要求：每种单据 ① 空白 Excel 底稿 .xlsx 1 份 + ② 真实已签发样张脱敏版 1 份，PDF 或 Excel 均可；参考「标准化外贸工作流」的 QT/SC 底稿模式，程序往固定单元格填数）
  - **进展（2026-08-11）：第 1 种「简要报价单」模板已到并落地**。`data/简要报价模板.xltx`（真实样张+空白模板两表，老板用「」占位符标注动态位；错字"标称英尺"已改"标称英寸"）。`tools/export_brief_quote.py`：`python3 tools/export_brief_quote.py <quote_no>` → `output/quotes/<quote_no>_简要报价.xlsx`。**老板确认的 6 条规则**：① 分组键=(产品类别,物料类型,公斤价)，同类别两种公斤价拆两组；② 卷价以库中 unit_price 为准（手填可偏离公式值，小偏差不提示）；③ 数值全从系统取（录入时 db_writer 已算好落库），模板只摆格式不在 Excel 里算；④ 合计行=卷数+体积+重量+金额（金额取 quotations.total_amount）；⑤ 明细行动态插行、合计/底部自动下移、格式复制模板行；⑥ .xltx 用 openpyxl 读，公式/合并/行高原样保留。单组（YL260728Q025-brief）与多组（YL260728Q025 三公斤价组）实测通过。**注意**：模板文件在 `data/` 下被 gitignore（含真实业务痕迹），部署 NAS 时随 `data/` 一起拷贝。
  - 待做：前端"导出报价单"按钮（调 API 生成+下载）；其余 5 种单据模板等老板提供
...
- [x] M1e [P1] **辅料模块自查修复**（2026-08-02 全面复查后修 4 处）：① **01_schema.sql 漂移**——aux_type ENUM 和 name 宽度没跟上 M1d 迁移，全新安装会建出旧表结构，已同步（五处同步纪律）；② **收发存辅料选择器混进 105+ 条纯档案**（包装/喷码都能选去出入库），前端改只拉 label_paper + 后端 aux_stock_move 加类型护栏（纯档案收发直接拒绝，实测 PK-001 出库被拦 ✓）；③ **新标签纸建档后收发存不可见**——aux_create_material 和自动建档两条路径都补上 AUX 仓零库存行（实测 LP-TEST99/LP-R88888 建档即带库存行 ✓），并回填了用户真实新标签 LP-R02507 的库存行；④ DATA_MODEL.md 辅料表描述同步。用户当日真实使用记录（M-Q025-013 + 6 条自动建档）确认功能上线即用，全部保留
- [x] M3b [P2] **标签需求提示·React 合同页**：随 F2.6 落地——合同落库成功自动调 label-demand，缺料红字 WARN 不阻止，实测 SC-PROXY01 算出 LP-R02505 需 10 缺 10 ✓
- [x] M1c [P1] **包装方式入辅料档案 + 录入页下拉**（2026-08-02 老板要求）：products.package 全部 102 种历史包装种子进 aux_materials（PK-001..102 按引用频次冻结快照，纯档案不计量、不进收发存，种子文件幂等复跑 ✓）；录入页"包装/标签纸"改辅料档案下拉（AutoComplete 可手填新值，标签纸显示"R码·名称"存原 R 编号）；auxMaterials 接口加 aux_type 过滤参数
- [x] M1d [P1] **手填值自动建档·总开关**（2026-08-02 老板要求一个对勾管全部）：aux_type ENUM 扩展 5 类工艺档案（spray_code/meter_mark/material_used/wire_pattern/coil_type，纯档案不计量，迁移 `2026-08-02_aux_types.sql`）+ name 加宽 255（喷码全文超 64 字符防截断，实测 105 字符完整入库 ✓）；AUTO_ARCHIVE_FIELDS 统一驱动 7 字段（包装/标签纸/喷码/米标/用料/打线/盘型），编码规则 LP-R码/其余 PREFIX-### 顺序；录入页改**一个总复选框**（提交行，默认开），喷码/米标/用料/打线下拉=档案+客户历史合并、盘型改档案下拉，建档成功自动刷新下拉；辅料档案页加类型筛选+建档表单可选类型。实测：7 字段一次全建档 ✓、重复值静默 ✓、开关关不建档 ✓（测试数据已清理）。**排障记录**：tools/ 是 ro 挂载进容器，**改 db_writer.py 必须 `docker compose restart api` 才生效**（uvicorn 进程缓存了旧模块，之前"即时生效"的说法是错的）
...
- [x] F2.6 [P2] **报价/合同/发货录入**（2026-08-02，录入端主线收官）：db_writer 新增单据层——suggest_doc_no（QT/SC/DN+日期+3位流水）、products-picker/quotations/contract-pending 选项接口、create_quotation/contract/delivery **头+明细单事务落库**（派生全部后端算：报价 unit_price=快照单重×系数、小计/总额/汇率带出/折CNY；合同单价直填；发货回写 delivered_qty+合同状态联动 completed/delivering）。闸门实测：快照重量覆盖 10kg 存行上、products.weight 8.5 不动（ADR-0005 ✓）；报价转合同 source_quote_no 标记 converted ✓；超发 999 拦截精确报未发 60 ✓；坏物料行整体回滚零泄漏 ✓；单号查重 ✓。前端 3 页+共享 DocItemsEditor（报价=系数+快照单重可改、合同=单价直填、发货=未发量带出默认全发）+ 合同页"从报价转入"预填 + M3b 落库后标签需求 Alert。8082 反代全链路实测（报价→合同→M3b→发货→合同 completed）✓，测试数据全部清理。**注意**：R11 公斤价反算三列录入时留 pending，由发货校验口径复核

---

### 第七组：主数据治理（2026-08-10/11 真实产品数据导入 + 客户/业务员编码体系）

> 背景：老板交付 14,338 行真实产品数据（`data/产品数据_整理后.xlsx`），借机完成产品目录全量核实、客户档案回填、客户编码规则落地、业务员档案上线。这是项目从"骨架+样例"转入"真实主数据"的里程碑。

- [x] G7.1 [P0] **产品目录全量核实**（14,338 行）：几何/id_x_od/体积零不符；重量超差 152 行保留客户值并 remark 标注（17 行"双连管按对计重"）；`CATEGORY_ALIASES` 补双空格变体 + 空格归一兜底；双联/双连、磨砂/磨沙等谐音异名三道防线（`tools/name_variants.py`，只认归一后相等，禁用编辑距离——实测误报：黄花园管≠蓝花园管）
- [x] G7.2 [P0] **体积口径统一**（老板拍板）：`volume = 外观外径(cm)²×高(cm)×0.93/1e6`（0.93 装箱系数；外观=整卷盘卷外径，非软管口径）；Q025 八款按 BL-2608 实测更新；校验第 8 步 ERROR→WARN（快照不随主数据修正）
- [x] G7.3 [P0] **客户档案回填**：products 664 种客户编号写法机械归一 → 601 个正式编号；`customers` 2→596 行（名称暂以编号占位）；垃圾码 8039/9129 系业务员更替沿革留置，其余置空；7 行孤儿产品待老板认领
- [x] G7.4 [P0] **客户编码规则落地（R12）**：字母+4位数字，首位=首录业务员数字编码，换业务员只换字母；`create_customer` 强校验 `^[A-Z]\d{4}$`；Q024/Q025 补全 Q0024/Q0025（FK NO ACTION：先补母行→子表改挂→删旧码）
- [x] G7.5 [P0] **业务员档案 `salespersons`**：建表+16 字母种子（按存量编号统计）+10 人姓名补录；`suggest_customer_code` 按业务员推荐；W 周世增 digit 9→3（避开 E/X/Z 重复，存量 W9xxx 不动、新客 W3001 起）
- [x] G7.6 [P0] **业务员档案管理页**（`/entry/salesperson`，录入中心第 10 张卡）：列表/建档/编辑一体；code/digit API 层锁死；`GET /api/salespersons` + `PUT /api/salespersons/{code}`；菜单补齐客户/入库/出库入口
- [x] G7.7 [P1] **编码推荐修复**：只认恰好 3 位流水合规码参与 max+1（D11150 类 5 位异常码曾把推荐带歪成 D11151）；流水满 999 返回空待老板定扩段
- [ ] G7.8 [P1] **客户真实名称整理**（等老板交付名单）：596 行占位名称回填 + **21 条 5 位异常码一并处理**（认领改合规码或置空）+ 剩余 6 个业务员姓名（C/E/K/P/T/Z）；7 行孤儿产品老板明确**不动**（2026-08-11）
...
| 前端工作台 | F1.1 ~ F2.6 / M1~M4 | 阶段一 Streamlit 已验收；阶段二 React 录入端 10 张卡全部上线 |
| 主数据治理 | G7.1 ~ G7.10 | 真实产品数据导入 + 客户编码 R12 + 业务员档案（2026-08-11） |
| ~~已砍~~ | ~~T2.6 / T2.10~~ | ~~audit_logs 提前做无意义 / 日志锦上添花~~ |

---

## 2b. 下一阶段工作计划（2026-08-11 主 agent 评估）

当前状态：录入端 10 张卡全部上线、真实主数据（14,338 产品 / 596 客户 / 16 业务员）在库、16 步校验全绿。建议优先级排序：

| 顺位 | 工作 | 类型 | 说明 |
| --- | --- | --- | --- |
| ① | **团队试用 + 反馈收集**（F1.5） | 人工 | 老板把 8082 录入端/8501 查询端发给业务经理和财务经理；真实用起来暴露的问题比任何测试都值钱 |
| ② | **FB.1 导入中心页**（React） | 开发 P1 | 批量 CSV 上传→校验→导入，补齐"录入靠页面、批量靠导入"的另一半；G7.8 客户名单回填正好走这条路 |
| ③ | **G7.8 客户真实名称整理** | 等老板 | 596 个占位名称 + 21 条 5 位异常码 + 6 个业务员姓名 + 7 行孤儿产品，一次批量回填 |
| ④ | **M4 辅料校验报表** | 开发 P2 | 辅料库存一致性 + 低库存预警进 16 步校验 |
| ⑤ | **F3.1 打印导出模板** | 等老板 | 报价单/合同/发货单/出库单/对账单/报关资料——等老板提供 ① 空白 Excel 底稿 ② 真实样张脱敏版 |
| ⑥ | T3.x 阶段二财务深化 | 暂缓 | AP/账龄/汇兑损益等录入端稳定后再动 |

> 原则：**先让真实业务在录入端跑起来，再回头补财务深化**。录入端还有真实使用暴露的问题时，不新开大模块。

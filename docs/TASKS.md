...
- [x] M1c [P1] **包装方式入辅料档案 + 录入页下拉**（2026-08-02 老板要求）：products.package 全部 102 种历史包装种子进 aux_materials（PK-001..102 按引用频次冻结快照，纯档案不计量、不进收发存，种子文件幂等复跑 ✓）；**录入页"包装"和"标签纸"改辅料档案下拉**（AutoComplete 可手填新值，标签纸显示"R码·名称"存原 R 编号）；auxMaterials 接口加 aux_type 过滤参数。**注意**：手填的新包装只进 products.package 不自动建档，要长期复用请在辅料档案页补建（或后续加"录入自动建档"开关）
